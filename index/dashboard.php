...
<body id="body-pd" class="body-pd">
  ...
  if (isset($_POST["item_per_page"])) {
    $items_per_page = $_POST["item_per_page"];
  ...
  if (!empty($_GET['item_per_page'])) {
    $items_per_page = filter_input(INPUT_GET, 'item_per_page', FILTER_VALIDATE_INT);
    $offset = ($page - 1) * $items_per_page;
    $limit = "LIMIT " . $offset . "," . $items_per_page;
  ...
  <div class="height-100  py-5">
        ...
        <div class="col">
          <form action="" method="GET">
            <div class="row d-flex flex-row align-items-center justify-content-start">
              <label class="col-3 form-label fs-5">แสดงรายการ</label>
              <input type="hidden" name="page" value=1>
              <select class="col form-label form-select fs-6" id="item_per_page" name="item_per_page" onchange="this.form.submit()">
                <option selected disabled><?= (isset($_GET["item_per_page"])) ? $_GET["item_per_page"] : 1 ?></option>
                <option value=1>1</option>
                <option value=2>2</option>
                <option value=3>3</option>
                <option value=4>4</option>
              </select>
              <label class="col-7 form-label fs-5">รายการ</label>
    ...
    <div class="col d-flex justify-content-end py-3">
          <div class="btn-group me-2" role="group" aria-label="page-nav">
            <?php
            $stmt2 = $pdo->prepare("SELECT COUNT(project_id) FROM project;");
            $stmt2->execute();
            $n = intdiv($stmt2->fetch()[0], $items_per_page);
            $i = 1;
            for ($i = 1; $i <= $n; $i++) {
              echo "<a type='button' class='btn btn-primary px-2' href='dashboard.php?page=$i&item_per_page=$items_per_page'>$i</a>";
            }
            ?>
          </div>
        </div>
